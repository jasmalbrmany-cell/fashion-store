...
async function scrapeWithFirecrawl(url: string): Promise<any | null> {
  ...
  try {
    ...
    // Build normalized result from extraction + markdown fallback
    const result = {
...
// ─── HTML Parsers (Layer 2 & 3 fallback) ─────────────────────────────────────
function extractImages(html: string): string[] {
  ...
  const addImage = (u: string) => {
    ...
    if (/50x50|32x32|16x16/.test(u)) return;
  ...
  [...html.matchAll(/data-(?:src|image|zoom|large|original)=["'](https?:\/\/[^"']+\.(?:jpg|jpeg|png|webp)[^"']*)/gi)].forEach(m => addImage(m[1]));
...
function parseHtml(html: string, url: string, currency = 'YER') {
  ...
  if (!price) {
    const pricePatterns = [/["']price["']\s*:\s*["']?(\d+[.,]?\d*)["']?/i, /(\d{2,6})\s*(?:ر\.ي|ريال|YER|SAR)/i];
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // ── Strategy 1: Firecrawl (best quality) ──
  ...
  // ── Strategy 2: Jina.ai reader (markdown rendering) ──
  ...
  // ── Strategy 3: CORS Proxy + HTML parse ──
  if (!result || (!result.title && result.images?.length === 0)) {
