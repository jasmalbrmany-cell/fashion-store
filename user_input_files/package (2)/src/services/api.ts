...
export const ordersService = {
  ...
  async updateStatus(id: string, status: string): Promise<Order | null> {
    if (!isSupabaseConfigured()) {
      ...
      if (index > -1) {
        mockOrders[index] = { ...mockOrders[index], status: status as any, updatedAt: new Date().toISOString() };
...
function transformProductToDb(product: Partial<Product>): Partial<ProductRow> {
  return {
    name: product.name,
